...
Editor.prototype = {
    ...
    insertBetween : function (json) {
        var begin = '<html><head><title>POM Example</title><style>body,html {margin: 0;padding: 0;border: 0;font-size: 100%;font: inherit;vertical-align: baseline;line-height: 1;}</style>';
		var scripts = [
			'_site/js/pixi.js',
			'_site/js/pom.min.js'
		];
		scripts.forEach(function(name) {
			begin += '<script src="'+name+'"></script>';
		});
		begin += '</head><body>';
        if (this.plugins) {
        ...
        begin += 'var data = ';
        var end = "var stage = new PIXI.Container();" +
